...
template <auto address, typename T = std::uint32_t> struct RegisterWriteOnly {
    template <auto offset, auto width>
    ...
    template <T... bits> static void set() {
        *reinterpret_cast<volatile T *>(address) |= ((1 << bits) | ...);
    }

    template <T... bits> static void clear() {
        *reinterpret_cast<volatile T *>(address) &= (~(1 << bits) & ...);
    }

    template <T... bits> static void toggle() {
        *reinterpret_cast<volatile T *>(address) ^= ((1 << bits) | ...);
    }
    ...
    template <typename... Pairs> static void write() {
        auto ptr = reinterpret_cast<volatile T *>(address);
        auto mask = (Pairs::Field::mask | ...);
        *ptr = (*ptr & ~mask) |
               (mask & ((Pairs::value << Pairs::Field::offset) | ...));
    }

    template <typename... FieldValues>
    static void write(FieldValues &&... fieldValues) {
        auto ptr = reinterpret_cast<volatile T *>(address);
        auto mask = (FieldValues::Field::mask | ...);
        *ptr =
            (*ptr & ~mask) |
            (mask & ((fieldValues.value << FieldValues::Field::offset) | ...));
    }
};

template <auto address, typename T = std::uint32_t>
struct Register : public RegisterReadOnly<address, T>,
